...
export default function Inventory() {
  ...
  return (
    <div>
      <h2>Street Light Inventory</h2>

      {/* Filter section */}
      <div>
        <label htmlFor="workingFilter">Filter by working status:</label>
      ...
      {/* Search section */}
      <div>
        <input type="text" placeholder="Search street light ID or address" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      ...
      <div>
        ...
        <button onClick={() => paginate(currentPage + 1)} disabled={currentLights.length < lightsPerPage}>Next</button>
      </div>
    </div>
  );
}
